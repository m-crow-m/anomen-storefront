...
export function AddToCartToast({ isVisible, productName, size, onClose }: AddToCartToastProps) {
  ...
  return (
    <div className="fixed top-24 left-1/2 z-50 w-[90%] max-w-md -translate-x-1/2">
      <div className="bg-black text-white border-4 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] transition-transform duration-300">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            <p className="font-heading uppercase tracking-wider mb-2">Added to Cart</p>
            <p className="text-sm">{productName}</p>
            {size && <p className="text-xs mt-1 opacity-80">{size}</p>}
          </div>
          <button
            onClick={onClose}
